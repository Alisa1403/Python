...
def winner_check(user, bots):
    global WINNER, count_winner_bot, count_winner_user
...
def stop(update, context):
    if WINNER == None:
        ...
        if WINNER == -1:
            ...
            context.bot.send_message(update.eÏffective_chat.id, f"{update.effective_user.first_name}, ты выиграл")
        elif WINNER == 0:
...
def count(update, context):
    global count_winner_bot, count_winner_user
    context.bot.send_message(update.effective_chat.id, f'Кол-во выигрышей у бота: {count_winner_bot}\n'
                                                       f'Кол-во выигрышей у {update.effective_user.first_name}: {count_winner_user}')

start_handler = CommandHandler('start', start)
still_handler = CommandHandler('more', more)
